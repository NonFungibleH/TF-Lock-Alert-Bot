...
// Module-level pool — reused across calls to avoid per-call TCP handshake overhead.
// Initialised lazily so the module loads safely without POSTGRES_URL.
let pool = null;
function getPool() {
...
async function getHuntrDbReputation(devWallet) {
  const p = getPool();
  if (!p) return { rugCount: 0, totalLocks: 0 };
