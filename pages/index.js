...
export default function Dashboard() {
  ...
  const formatTxHash = (txHash) => {
    ...
    return txHash.length > 10 ? `${txHash.substring(0, 6)}...${txHash.substring(txHash.length - 4)}` : txHash
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-purple-900 to-gray-900 text-white">
      <div className="container mx-auto px-4 py-8">
                ...
                {filteredLocks.length === 0 ? (
                  <tr>
                    <td colSpan="9" className="px-6 py-8 text-center text-gray-400">
                      {locks.length === 0 ? 'No locks detected yet' : 'No locks found matching your filters'}
                    </td>
                  </tr>
                ) : (
                  filteredLocks.map((lock, index) => {
                    const performance = calculatePerformance(lock.lockPrice, lock.currentPrice)
                    const shortHash = formatTxHash(lock.txHash)
                    ...
                    return (
                      <tr key={index} className="border-t border-purple-500/10 hover:bg-purple-500/5">
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            <div className="w-6 h-6 bg-gradient-to-br from-purple-500 to-pink-500 rounded-full flex-shrink-0"></div>
        ...
        <div className="mt-8 text-center text-gray-400">
          ...
          {locks.length > 0 && (
            <p className="text-xs mt-2">Auto-refreshes every 30 seconds • Real-time lock detection active</p>
          )}
        </div>
      </div>
    </div>
